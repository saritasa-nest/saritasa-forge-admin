...
public class EditEntityViewModel : ValidationEntityViewModel
{
    ...
    public override async Task LoadAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                var pocoInstance = ProxyToPocoConverter.ConvertProxyToPoco(instance, includedNavigationNames);
                instance = pocoInstance;
            }

            // We have to keep the original entity to work with navigation properties.
            Model.OriginalEntityInstance = ObjectCloner.Clone(instance);
    ...
    public async Task UpdateEntityAsync()
    {
        foreach (var (property, file) in filesToUpload)
        {
            ...
            Model.ChangesTracker.SetPropertyValue(property.Name, fileString);
        }
        ...
        if (!entityService.ValidateEntity(Model.ChangesTracker!, Model.Properties, ref errors))
        {
        ...
        try
        {
            var updatedEntity = await dataService.UpdateAsync(
                    Model.ChangesTracker.TrackingObject!,
            ...
            var instance = ObjectCloner.Clone(updatedEntity);
            Model.ChangesTracker = new(instance);
